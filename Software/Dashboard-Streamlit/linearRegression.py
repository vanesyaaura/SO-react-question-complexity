...
# Fungsi untuk menginstal library yang dibutuhkan
def install_package(package):
    subprocess.check_call([sys.executable, "-m", "pip", "install", package])

# Cek dan install package jika belum terinstal
try:
    import matplotlib.pyplot as plt
except ImportError:
...
if df_reputasi.empty:
    st.warning("Data tidak tersedia. Pastikan CSV sudah diunggah dengan benar.")
    st.stop()
elif 'Reputation' not in df_reputasi.columns or 'pd_score' not in df_reputasi.columns:
    st.error("Kolom 'Reputation' atau 'pd_score' tidak ditemukan pada dataset.")
    st.stop()
...
if choice == "Data Overview":
    ...
    ax5.plot(X_test, y_pred_test, color='firebrick', label='Predicted')
